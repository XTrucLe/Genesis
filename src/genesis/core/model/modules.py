...
class GroupedQueryAttention(nn.Module):
    def __init__(
        self, dim: int, heads: int, kv_heads: int, block_size: int, dropout: float = 0.1
    ):
        super().__init__()
        assert dim % heads == 0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.num_queries_per_kv > 1:
            ...
            k = k.unsqueeze(2).expand(
                B, self.kv_heads, self.num_queries_per_kv, T, self.head_dim
            )
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = GroupedQueryAttention(
            dim, heads, kv_heads=kv_heads, block_size=block_size, dropout=dropout
        )
